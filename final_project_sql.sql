...
Query OK, 0 rows affected (0.570 sec)
...
mysql> SELECT* FROM Students WHERE EnrollmentDate > '2022-12-31';
Empty set (0.009 sec)

mysql> SELECT c.* FROM Courses c JOIN Departments d ON c.DepartmentID = d.DepartmentID
...
mysql> SELECT StudentID
    ...
    -> FROM Students;
ERROR 1064 (42000): You have an error in your SQL syntax; check the manual that corresponds to your MySQL server version for the right syntax to use near '(EnrollmentDate) AS EnrollmentYear
FROM Students' at line 2
mysql>
mysql> SELECT StudentID,
    -> YEAR(EnrollmentDate) AS EnrollmentYear
    -> FROM Students;
